...
/*
	string command(msg->getMsg(),msg->getSize());

	unsigned int offset;
	if (((offset = command.find("list"))) < command.size() )
	{
		int fd;

		for (fd=0;fd<64*1024;fd++)
		{
			int32_t iError = 0;
			int32_t iSize = sizeof(iError);


			if (getsockopt(fd,SOL_SOCKET, SO_ERROR, &iError,(socklen_t *) &iSize) == 0 )
			{
				if (iError != 0)
                	continue;

				struct sockaddr_in addrLocal;
				iSize = sizeof(addrLocal);

				if ( getsockname(fd, (struct sockaddr *) &addrLocal, (socklen_t *) &iSize) != 0 )
					continue;

				struct sockaddr_in addrRemote;
				iSize = sizeof(addrRemote);
				if ( getpeername(fd, (struct sockaddr *) &addrRemote, (socklen_t *) &iSize) != 0 )
					continue;


				string localhost,remotehost;

				localhost = inet_ntoa(*(struct in_addr *)&((sockaddr_in)addrLocal).sin_addr);
				remotehost = inet_ntoa(*(struct in_addr *)&((sockaddr_in)addrRemote).sin_addr);
